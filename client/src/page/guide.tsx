...
export function GuidePage() {
  ...
  const results = useMemo(() => {
    ...
    if (savedOnly) return entries.filter(item => saved.includes(item.id));
  ...
  }, [data, selectedId, savedOnly, saved, query, category, evidence]);
  ...
  return <main className="guide-page ani-show"><Helmet><title>Life Guide - {process.env.NAME}</title></Helmet><header><div><p>LIFE · LOW-NOISE KNOWLEDGE</p><h1>{savedOnly ? 'Saved Advice' : 'Guide'}</h1><span>需要时再查，不把建议变成新的任务清单。</span></div><Link href="/life">返回 Life</Link></header>{!savedOnly && !selectedId && <section className="guide-tools"><label><i className="ri-search-line" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索生活问题……" /></label><div>{categories.filter(name => ['时间与精力','放松与娱乐','学习与技能','信息与平台安全','急救与应急','旅行安全','财产与法律风险'].includes(name)).map(name => <button key={name} className={category === name ? 'active' : ''} onClick={() => setCategory(category === name ? '' : name)}>{name}</button>)}</div><select value={evidence} onChange={event => setEvidence(event.target.value)}><option value="">全部证据等级</option><option value="A">Evidence A</option><option value="B">Evidence B</option><option value="C">Evidence C</option></select></section>}<section className="guide-results">{results.map(advice => <article key={advice.id}><div className="guide-card-meta"><span>{advice.category}</span><b>Evidence {advice.evidence}</b></div><h2>{advice.title}</h2><p>{advice.summary}</p><details open={Boolean(selectedId)}><summary>成本、收益与来源</summary><dl><dt>成本</dt><dd>{advice.cost || '未单独列出'}</dd><dt>收益</dt><dd>{advice.benefit || advice.summary}</dd>{advice.detail && <><dt>备注</dt><dd>{advice.detail}</dd></>}</dl><a href={advice.sourceUrl} target="_blank" rel="noreferrer">查看原始条目 <i className="ri-external-link-line" /></a></details><div className="guide-card-actions"><button onClick={() => toggleSaved(advice.id)}><i className={saved.includes(advice.id) ? 'ri-bookmark-fill' : 'ri-bookmark-line'} /> {saved.includes(advice.id) ? '已收藏' : '收藏'}</button><button onClick={() => openHabit(advice)}>转为习惯</button></div></article>)}{results.length === 0 && <p className="guide-empty">{savedOnly ? '还没有收藏建议。' : '输入问题或选择分类后，结果才会出现。'}</p>}</section>{message && <p className="guide-message">{message}</p>}{habitAdvice && <div className="guide-modal" role="dialog" aria-modal="true"><section><p>创建 Habit</p><h2>{habitAdvice.title}</h2><label>名称<input value={habitName} onChange={event => setHabitName(event.target.value)} /></label><div><button onClick={() => setHabitAdvice(null)}>取消</button><button onClick={createHabit}>创建</button></div></section></div>}</main>;
}
