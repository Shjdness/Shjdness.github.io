...
function RssView() {
  ...
  const selectedSource = data?.subscriptions.find(source => source.id === sourceId) || null;
  ...
  const load = async (append = false) => {
    ...
    try {
      const query = new URLSearchParams({ filter: sourceId ? 'all' : filter, contentType, limit: '90', offset: String(nextOffset) });
  ...
  const refreshAll = async () => { if (busy) return; setBusy(true); try { const result = await lifeApi<{ refreshed: number; added: number; failed: number }>('/rss/refresh', { method: 'POST' }); setMessage(`已更新 ${result.refreshed} 个来源，新增 ${result.added} 条${result.failed ? `，${result.failed} 个来源失败` : ''}。`); await load(); } catch { setMessage('更新失败，已缓存内容仍可阅读。'); } finally { setBusy(false); } };
  ...
  const refreshSource = async (source: RssSubscription) => { setBusy(true); try { const result = await lifeApi<{ added: number }>(`/rss/subscriptions/${source.id}/refresh`, { method: 'POST' }); setMessage(`「${source.alias || source.title}」新增 ${result.added || 0} 条。`); await load(); } finally { setBusy(false); } };
