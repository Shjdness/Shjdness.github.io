...
async function sendQueuedMutation(item: { entity: string; action: string; payload: unknown }) {
  if (item.entity === 'habit' && item.action === 'create') {
    ...
    const { data, error } = await withTimeout<any>(client.habit.index.post({ name: payload.name, description: payload.description, color: payload.color, clientKey: payload.clientKey }, { headers: headersWithAuth() }) as Promise<any>, WRITE_TIMEOUT);
