...
function RssView() {
  ...
  const cacheKey = `life:rss:v4:${sourceId || filter}:${contentType}`;
  ...
  useEffect(() => { setOffset(0); setSelectedItem(null); void load(); }, [filter, contentType, sourceId]);
  ...
  const embedFor = (item: RssItem) => {
    ...
    const url = new URL(item.embedUrl); url.searchParams.set('playsinline', '1'); url.searchParams.set('rel', '0'); url.searchParams.set('origin', window.location.origin); return url.toString();
  };
      ...
      <aside className="rss-library">
        ...
        <div className="rss-library-content">
          <section className="rss-sidebar-card">
            ...
            <button className={!sourceId && filter === 'unread' ? 'active' : ''} onClick={() => { setSourceId(null); setFilter('unread'); }}><i className="ri-mail-unread-line" /><span>未读</span><b>{counts.unread}</b></button>
            <button className={!sourceId && filter === 'starred' ? 'active' : ''} onClick={() => { setSourceId(null); setFilter('starred'); }}><i className="ri-star-line" /><span>收藏</span><b>{counts.starred}</b></button>
            {!sourceId && filter === 'unread' && counts.unread > 0 && <button className="rss-clear-unread" onClick={() => void markAllRead()}><i className="ri-check-double-line" /><span>一键清除未读</span></button>}
          </section>
          <section className="rss-sidebar-card">
